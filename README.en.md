...
The current version follows the npm registry `latest` dist-tag. The ecosystem publishes 24 npm targets: 19 standard component / core / renderer / preset packages and 5 historical aliases. New integrations should prefer the `@file-viewer/*` standard package names; existing applications using `@flyfish-group/*` or `file-viewer3` continue to receive the same versioned capability set.
