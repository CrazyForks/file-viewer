...
The current version follows the npm registry `latest` dist-tag. The ecosystem publishes 21 npm targets: 16 standard component / core / renderer / preset packages and 5 historical aliases. New integrations should prefer the `@file-viewer/*` standard package names; existing applications using `@flyfish-group/*` or `file-viewer3` continue to receive the same versioned capability set.
