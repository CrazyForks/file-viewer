...
`@file-viewer/web-full` CDN/IIFE distributions are the exception: direct jsDelivr/unpkg usage, or an intact deployment of the complete `dist/` directory under one static prefix, resolves renderer, Worker, WASM, font, and vendor assets relative to the script URL and needs no copy command. Copying only the entry IIFE is not a complete deployment.
