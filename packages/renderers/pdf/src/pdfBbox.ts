...
const finitePositive = (value: unknown) => Number.isFinite(value) && Number(value) > 0;

export const normalizePdfBoundingBoxInput = (
  input: unknown
): FileViewerPdfBoundingBox[] => {
  const values = input ? (Array.isArray(input) ? input : [input]) : [];
  return values.filter(value => !!value && typeof value === 'object') as FileViewerPdfBoundingBox[];
};

export const normalizePdfBoundingBox = (
  input: FileViewerPdfBoundingBox,
  pageBox: PdfPageBox,
  fallbackPage = 1
): NormalizedPdfBoundingBox | null => {
  if (
...
export const rotateNormalizedPdfBoundingBox = (
  box: NormalizedPdfBoundingBox,
  rotation: number
): NormalizedPdfBoundingBox => {
...
export const serializePdfBoundingBoxes = (
  input: unknown
) => JSON.stringify(normalizePdfBoundingBoxInput(input));
