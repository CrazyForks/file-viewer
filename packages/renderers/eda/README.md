...
- GDSII/OASIS 底层能力已经下沉到 `@file-viewer/eda-layout`；OrCAD/Allegro 二进制检查能力已经下沉到 `@file-viewer/eda-orcad`。本包只负责 UI 展示和 File Viewer renderer 协议。
...
当前 EDA renderer 没有额外 Worker/WASM 静态资产。它只在命中 OLB、DRA、GDSII 或 OASIS 格式时加载 `cfb`、`@file-viewer/eda-layout`、`@file-viewer/eda-orcad` 和本包 UI 代码。
