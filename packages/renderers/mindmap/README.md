...
Flyfish File Viewer 的独立 XMind / Mind Map renderer 包。它负责解析现代 `content.json` 和经典 `content.xml` XMind 文件，并提供可拖拽、可缩放、可定位的脑图画布。
...
- 支持工具栏缩放、适配画布、按住鼠标或触摸拖拽平移、`Ctrl` / `Command` + 滚轮指针缩放、键盘方向键平移。
