...
- **接入快。** Vanilla JS、Vue、React、Svelte、jQuery 都有原生组件；full 包内置完整 renderer 矩阵，并统一使用一套运行时资产契约。
...
`*-full` 表示完整 renderer 矩阵已内置，不要再安装或传入其它 preset。完整格式支持还包括配套 Worker、WASM、字体和 vendor 资源：Vite 使用 `fileViewerRenderers({ copyAssets:true })`，自动发布到部署基址下的 `file-viewer/`（根部署即 `/file-viewer/`）；其他构建工具运行每个 full 包自带的同版本 `npx --no-install file-viewer-copy-assets ./public/file-viewer`。缺少该目录时，轻量格式和少数兼容路径可能仍能工作，但不属于完整格式支持。直接使用 CDN `web-full`，或完整镜像它的整个 `dist/` 目录时，无需额外复制。
...
所有 full 包默认把 Archive、PDF、DOCX、Excel、PPTX、CAD、Typst、Draw.io、SQLite 等运行时资产指向部署基址下的 `file-viewer/`（根部署即 `/file-viewer/`）。Vite 自动发布或复制 CLI 写入 `./public/file-viewer` 后，不需要逐项手写资源 URL。
...
当前版本以 npm registry 的 `latest` dist-tag 为准，共维护 54 个 npm 发布目标: 48 个标准组件/完整 full 包/核心/renderer/preset/工程插件包 + 6 个历史兼容 alias。新项目建议优先使用 `@file-viewer/*` 标准包名；旧项目继续使用 `@flyfish-group/*` 或 `file-viewer3` 时也会拿到同版本能力。
...
Core、PPTX 原生引擎、Vanilla JS / Pure Web、Vue3、Vue2、React、React legacy、jQuery、Svelte 和历史兼容 tarball 都会随开源总仓库一起生成。`file-viewer3` 非 scoped 兼容包仍会同步发布到 npm，但它和 `@flyfish-group/file-viewer3` 包体重复，开源总仓库不再重复存储该 tarball。非 Vite full 项目使用随包安装的同版本 CLI 发布完整资源：`npx --no-install file-viewer-copy-assets ./public/file-viewer`；`web-full` 完整部署其 `dist/` 也可直接使用。
