...
浏览器 Worker 不能直接操作真实页面 DOM，因此这里采用“Worker 构建 docx-preview HTML + 主线程分批挂载”的标准链路，而不是自研简化版 Word 渲染器。极端超大或结构异常的 Word 文件仍建议业务侧提供原文件下载，或在服务端转为 PDF/OFD 后再做稳定版式预览。
