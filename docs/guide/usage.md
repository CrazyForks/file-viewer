...
`geojson` 会直接按 GeoJSON 读取；`kml` 和 `gpx` 会由 `@file-viewer/renderer-geo` 按需加载 `@tmcw/togeojson` 转换；`shp` 会由同一个 renderer 包按需加载 `shpjs`。内置预览使用离线 MapLibre 空底图渲染点线面叠加层，不依赖在线瓦片服务，适合内网附件中心快速确认点位、轨迹和边界。
